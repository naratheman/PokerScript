...
const semanticChecks = [
  ...
  ["return", "straddle chip f()$. cashout.$"],
  ["if statements", "excuses x==0 $. chip x :10.$"],
  [
    "else if",
    "excuses x==0 $. chip x :10 .$ noMoreExcuses $. stringBet y: no .$",
  ],
  ["for loop", "playingLoose (chip x:0, x<= 10, x+$) $. reveal 'Hello' .$ ;}"],
  ["conditionals with ints", "reveal'true ? 8 : 5';"],
  ...
  ["subscript exp", "flop chip a: [1,2]; reveal a[0];"],
  ...
  ["array parameters", "straddle chip f(flop chip : x) $. .$"],
  ["optional parameters", "straddle chip f(chip? : x, stringBet? : x) $. .$"],
...
const semanticErrors = [
  ...
  // ],
  ["non-int increment", "playingOnTilt x: hit  x+$;", /an integer/],
  ["non-int decrement", "playingOnTilt x: miss  x-$;", /an integer/],
  ["undeclared id", "reveal 'x';", /Identifier x not declared/],
  ["redeclared id", "chip x : 1 chip x : 1;", /Identifier x already declared/],
  // ["recursive struct", "struct S { x: int y: S }", /must not be recursive/],
  [
    "assign to const",
    "constantPressure chip x : 1 x : 2;",
    /Cannot assign to constant x/,
  ],
  ["assign bad type", "chip x:1 x:true;", /Cannot assign a boolean to a int/],
  [
    "assign bad array type",
    "chip x:1 flop playingOnTilt x:hit ;",
